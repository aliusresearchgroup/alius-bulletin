...
- Treat notable quotes as semantic Q&A-segment inserts. Do not hand-freeze a blank-producing quote position; run `python AI-agents/layout_notable_quotes.py` so generated coordinates prefer the quote immediately after the question when the segment can absorb it locally, otherwise keep it later in the same segment without stranding large whitespace, while using word-boundary line breaks, close quote-mark framing, and compact one-line vertical gaps.
