...
- Treat notable quotes as semantic Q&A-segment inserts. Do not hand-freeze a blank-producing quote position; run `python AI-agents/layout_notable_quotes.py` so generated coordinates keep the quote horizontally centered on the PDF page, near its source sentence, after its source answer material and before the next question without stranding large whitespace, while using word-boundary line breaks, close quote-mark framing, and compact one-line vertical gaps.
- Treat decorative pull quotes as framed text boxes: run `python AI-agents/standardize_pull_quote_blocks.py` after extraction/regeneration so opening and closing quote marks sit outside the page-centered quote body instead of becoming their own lines or landing between quote-body lines.
