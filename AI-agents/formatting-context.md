...
- Notable quote policy: every interview source exposes `\ALIUSMaybeNotableQuoteAt{x}{y}{width}{quote text}` for optional per-Q&A-segment amplified excerpts. Leave the fourth argument empty to skip the slot. A notable quote belongs inside the segment it amplifies: after the question and the relevant answer text, before the next question, and only in vertical whitespace that would otherwise be idle. Do not compress surrounding answer text to force a quote; either skip the quote or move it to a later safe slot in the same segment.
...
- Use `\ALIUSMaybeNotableQuoteAt{x}{y}{width}{...}` inside the page's `tikzpicture`. The macro renders only when the quote argument is non-empty, so empty slots are allowed and should visually disappear.
- Placement rule: put the quote after the sentence or paragraph it amplifies and before the next green question. Prefer bottom-of-segment whitespace; avoid placing a quote across a page footer, page header, citation panel, author/contact column, or abstract block.
