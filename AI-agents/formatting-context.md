...
- First-page citation panels have a uniform structure across issues: white panel background over the left citation column, citation text anchored top-left, and the DOI anchored bottom-right. The panel deliberately stops before the right-side author/contact column and before the abstract/heading zone to prevent overlap with neighboring text elements.
