...
- Future semantic quote inserts use `\ALIUSPullQuoteBlockAt` through `\ALIUSNotableQuoteAt` / `\ALIUSMaybeNotableQuoteAt`; the macro places quote marks as overlay nodes around a centered minipage so the marks do not create line breaks inside the quoted text.
...
- After regenerating interview TeX files, run `python AI-agents/enforce_quote_and_doi_invariants.py`, then `python AI-agents/standardize_pull_quote_blocks.py`, then `python AI-agents/validate_quote_and_doi_rendering.py`. When issue PDFs are compiled, add `--compiled-dir <build-dir>` to verify live PDF link annotations.
