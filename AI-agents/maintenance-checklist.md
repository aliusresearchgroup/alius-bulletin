...
- [ ] `AI-agents/smarten_visible_quotes.py --check` has been run after text repair/regeneration, and visible interview prose still uses typographic apostrophes and quotation marks rather than drifting back to ASCII placeholders.
...
- [ ] `AI-agents/audit_pdf_text_fidelity.py` has been run when the task is article-body fidelity against originals; review `tmp/pdf-text-fidelity/manual-review.md` honestly, with special care for extraction-order artifacts caused by pull quotes, citation panels, abstracts, and hidden links.
...
- [ ] `AI-agents/standardize_pull_quote_blocks.py --check` passes: extracted pull quotes have page-centered body rectangles, opening marks on the first body baseline, closing marks on the final body baseline, no cross-page continuation that could fit on the opener page, and no spacing warnings.
- [ ] `AI-agents/layout_notable_quotes.py --check` passes after any notable-quote edit: quotes are horizontally page-centered, remain inside their Q&A segment near their source sentence, do not collide with fixed headers/footers, do not strand large blank bands before continued segment text, use compact one-line vertical gaps, and avoid hyphenated line breaks by using generated word-boundary line breaks.
...
- [ ] Optional notable quotes are represented as semantic Q&A quote slots; their `\ALIUSMaybeNotableQuoteAt{x}{y}{width}{quote}` coordinates are generated by the quote-layout rule pass rather than manually frozen as editorial placement.
